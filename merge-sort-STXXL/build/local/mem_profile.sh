...
if [ "${args[4]}" = "2" ]; then
	...
	nohup cgexec -g memory:cache-test ./merge_sort_strings ${args[0]} ../../../database_sample/DB_${args[1]} temp &
 #>>results-${args[0]}-${args[1]}-${args[2]}-${args[3]}-${args[4]} &
...
if [ "${args[2]}" = "1" ]; then 
	echo "Running Memory Profile 1: No change to "${args[0]}", enter a dummy value for "${args[3]}
#>results-${args[0]}-${args[1]}-${args[2]}-${args[3]}-${args[4]}
	let M=${args[0]}*1024*1024
	echo $M > /var/cgroups/cache-test/memory.limit_in_bytes
	wait $PID
