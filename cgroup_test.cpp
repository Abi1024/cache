...
const bool double_memory = false;
...
void mm( TYPE* x, TYPE* u, TYPE* v, int n )
{
    ...
    if (limit == 0){
      print_io_data(io_stats, "Printing I/O statistics at middle of multiplication \n");
      if (double_memory){
              limit_memory(starting_memory*2,cgroup_name);
      }else{
        limit_memory(starting_memory/2,cgroup_name);
      }
...
int main(int argc, char *argv[]){
  ...
  std::cout << "Cgroup name: " << cgroup_name << std::endl;
...
*/
  ...
  limit_memory(std::stol(argv[1])*1024*1024,cgroup_name);
